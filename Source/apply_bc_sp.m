function gamma = apply_bc_sp(params,gamma,gamma0,velocity)
    ...
    switch nargin
        case 2
            gamma.x(1,:) = 0;
            gamma.x(2:Nx,1) = gamma.x(2:Nx,2);
            gamma.x(2:Nx,Ny+1) = gamma.x(2:Nx,Ny);
            gamma.x(Nx+1,:) = 0;
        case 3
            gamma.x(1,:) = 0;
            gamma.x(2:Nx,1) = gamma.x(2:Nx,2);
            ...
            for i = Nx+1
                for j = 1:Ny+1
                    gamma.x(i,j) = gamma0.x(i,j) - params.Co * (gamma0.x(i,j)-gamma0.x(i-1,j));
                end
            end
        case 4
            gamma.x(1,:) = 0;
            ...
            for i = Nx+1
                for j = 1:Ny+1
                    gamma.x(i,j) = gamma0.x(i,j) - U_inf(i,j-1) * params.dt/params.dx * (gamma0.x(i,j)-gamma0.x(i-1,j));
                end
            end
    end
end